preprocessing <- function(data) {
  ...
  scopus_cleaned <-
    ...
    dplyr::select(AU, DE, ID, C1, CR, JI, AB, PA, AR, chemicals_cas,
                  ...
                  LA, TC, PN, page_count, PP, PU, PM, DB, sponsors, TI, url,
...
get_citation_network <- function(data_preprocesed) {
  ...
  sca_data_cleaned_1 <- 
    ...
    select(name = SR, TI) |>
    na.omit() |> 
    bind_rows(data_preprocesed |> 
                filter(!is.na(SR_ref)) |> 
                select(name = SR_ref, TI) |>
                unique() |> 
                na.omit() |> 
...
get_citation_network_tos <- function(citation_network) {
  ...
  for (i in 1:length(subfields$subfield) ) {
  ...
  sca_citation_network_tos <- 
    citation_network |> 
    activate(nodes) |> 
    left_join(df_tos, by = c("name", "subfield"))
  # Export year!!
